...
if __name__ == '__main__':
    ...
    model = AutoPeftModelForCausalLM.from_pretrained(
        args.checkpoint, # path to the output directory
        device_map="cuda",
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.checkpoint,
                                              trust_remote_code=True)
    model.generation_config = GenerationConfig.from_pretrained('/mnt/petrelfs/lijingsong/MLLM/Models/Qwen-VL-Chat', trust_remote_code=True)
    ...
    results = []
    fout = open('result_' + args.job_name + '_{}.jsonl'.format(torch.distributed.get_rank()), 'w')
    with torch.no_grad():
