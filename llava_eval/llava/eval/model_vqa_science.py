...
def eval_model(args):
    ...
    for i, line in enumerate(tqdm(questions)):
        ...
        if args.single_pred_prompt:
            ...
            # cur_prompt = cur_prompt + '\n' + "Answer with the option's letter from the given choices directly. Check carefully to make sure your answer is correct."


        if 'image' in line:
            image_file = line["image"]
            img_path = os.path.join(args.image_folder, image_file)
            cur_prompt = f'Picture 1: <img>{img_path}</img>\n{cur_prompt}'
        query = cur_prompt
